...
{
  QString filepath = QString("assets/reactions/%1").arg(ReactionPanel::getReactionAssetName(reaction));
  ...
  int y = QRandomGenerator::global()->bounded(height() - reactionLabel->height());
  ...
  connect(fadeOut, &QPropertyAnimation::finished, [reactionLabel, fadeOut, effect]() {
