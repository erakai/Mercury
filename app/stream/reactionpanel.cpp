...
ReactionPanel::ReactionPanel(QWidget *parent)
{
  ...
  for (int i = 0; i < rows; i++)
  {
    for (int j = 0; j < cols; j++)
    {
      ...
      QPixmap pix(QString("assets/reactions/reaction_%1.png").arg(QString::fromStdString(reactionStringList[index])));
  ...
  // reaction_panel_label->setStyleSheet("color: #dddddd; font-size: 18px; padding: 8px; border-radius: 10px; background-color: #222222;");
  // reaction_panel_label->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
  // reaction_panel_label->setText("Reactions");
  //
